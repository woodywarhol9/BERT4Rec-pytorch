...
class MultiHeadedAttention(nn.Module):
    ...
    def forward(self, q, k, v, mask=None):
        ...
        query = query.view(batch_size, -1, self.head_num, self.head_dim).permute(
            0, 2, 1, 3
        )
        key = key.view(batch_size, -1, self.head_num,
                       self.head_dim).permute(0, 2, 1, 3)
        value = value.view(batch_size, -1, self.head_num, self.head_dim).permute(
            0, 2, 1, 3
        )
        # attention 스코어 계산
...
class TransformerEncoder(nn.Module):
    ...
    def __init__(
        self, hidden_dim, head_num, ff_dim, dropout_rate=0.1, dropout_rate_attn=0.1
    ):
        """
...
class BERT(nn.Module):
    ...
    def __init__(
        ...
        self.embedding = BERTEmbeddings(
            vocab_size=self.vocab_size, embed_size=self.hidden_dim, max_len=self.max_len
        )
        # Transformer Encoder
